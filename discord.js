...
if (message.content.startsWith('!bcall')){
 if (message.author.id !== '522331271406551050') return message.reply('** هذا الأمر قفط لصاحب البوت و شكراًً **')
...
if (message.content.startsWith(prefix + `!قطوه`)) {
...
if(message.content.startsWith('!مستخدم') ) {
     if(!message.channel.guild) return message.reply('** غير مصرح لك أستخدام هذا الأمر ! **')
...
.then(collected =>{ 
 ...
 client.on("message", message => {
    if(message.content.startsWith(prefix + "سيجي")) {
...
message.channel.send('**لقد أرسلت إليك أوامر السيرفر بالخاص**');
          ...
          .then(co => {
            text = co.first().content
              ...
              message.channel.send(`تم حفظ اقتراحك الرجاء انتضار الرد من قبل الاداره`)
                client.channels.get("523124024549572610").send(`${message.author.username}'s أقتراحه هو => ${text}`)
...
client.on('message',function(message) {
  if (message.author.bot) return;
                  ...
                  if(!message.channel.guild) return;
                    ...
                    if (message.content === prefix + "الأعضاء") {
 const embed = new Discord.RichEmbed()
...
client.on('message', message => { 
let PREFIX = '!'
    if (message.content.startsWith(PREFIX + 'الأيقونات')) {

        const List = message.guild.emojis.map(e => e.toString()).join(" ");

        const EmojiList = new Discord.RichEmbed()
            .setTitle('➠ أيقونات السيرفر') 
            .setAuthor(message.guild.name, message.guild.iconURL) 
            .setColor('RANDOM') 
            .setDescription(List) 
...
client.on('message',function(message) {
   ...
   if(message.content.startsWith(prefix + "عد")) {
